...
class JetID : public Selector<int>  {
  ...
  version_(version), quality_(quality)
 {
  ...
  // 
  bool firstDataCuts( const int & jet,
		      pat::strbitset & ret) 
  {    
...
//    reco::BasicJet const * basicJet = dynamic_cast<reco::BasicJet const *>(&jet);
	...
	chf           = jetInfo_.CHF[jet] ; 
	nhf           = jetInfo_.NHF[jet] ; 
	cef           = jetInfo_.CEF[jet] ; 
	nef           = jetInfo_.NEF[jet] ; 
	nch           = jetInfo_.NCH[jet] ; 
	nconstituents = jetInfo_.NConstituents[jet] ; 
...
//    if ( ignoreCut(indexNCH_)           || ( nch > cut(indexNCH_, int())    || std::abs(jet.eta()) > 2.4 ) ) passCut( ret, indexNCH_);    
    ...
    if ( ignoreCut(indexNHF_)           || ( nhf < cut(indexNHF_, double()) ) ) passCut( ret, indexNHF_);    
    // Cuts for |eta| < 2.4:
    if ( ignoreCut(indexCEF_)           || ( cef < cut(indexCEF_, double()) || std::abs(jetInfo_.Eta[jet]) > 2.4 ) ) passCut( ret, indexCEF_);
    if ( ignoreCut(indexCHF_)           || ( chf > cut(indexCHF_, double()) || std::abs(jetInfo_.Eta[jet]) > 2.4 ) ) passCut( ret, indexCHF_);
    if ( ignoreCut(indexNCH_)           || ( nch > cut(indexNCH_, int())    || std::abs(jetInfo_.Eta[jet]) > 2.4 ) ) passCut( ret, indexNCH_);    
